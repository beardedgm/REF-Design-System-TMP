# Base Style Guide — Signature Aesthetic

A complete, app-agnostic visual design system. Use this document as the master reference to recreate a consistent, refined experience across all future applications, dashboards, and landing pages. Supports both **dark** and **light** themes via `data-theme` attribute on `<html>`.

---

## 1. Design Philosophy

* **Dark-First, Light-Ready:** Deep navy/charcoal backgrounds are the default canvas, with a full light theme available via `[data-theme="light"]`. All tokens are theme-aware — reference variables, never raw hex values.
...
All tokens below replace their dark counterparts when `data-theme="light"` is set on the root element. Components reference the same CSS variable names — only the values change.

**Backgrounds (Lightest → Deepest)**
* `--bg-primary` (`#f4f4f6`): Application background.
* `--bg-secondary` (`#eaeaee`): Alternating sections, secondary surfaces.
* `--bg-surface` (`#ffffff`): Input fields, inner surface areas.
* `--bg-card` (`#ffffff`): Cards, modals, panels.
* `--bg-elevated` (`#f0f0f4`): Hover states, popovers.

**Text & Content**
* `--text-primary` (`#1a1b24`): Headings, main body text.
* `--text-secondary` (`#5c5d6e`): Labels, descriptions.
* `--text-tertiary` (`#72738a`): Metadata, tertiary text.
* `--text-muted` (`#9394a5`): Hints, placeholders.
* `--text-on-accent` (`#ffffff`): Light text on accent backgrounds (inverted from dark mode).

**Signature Accent (Deepened for light-bg contrast)**
* `--accent` (`#a8843e`): Primary buttons, active tabs, main links.
* `--accent-hover` (`#c9a267`): Hover state.
* `--accent-muted` (`rgba(168, 132, 62, 0.10)`): Subtle highlights, focus rings.

**Semantic States (Darkened for readability)**
* `--error` (`#cd2b31`)
* `--success` (`#218358`)
* `--warning` (`#c47d0a`)
* `--warning-muted` (`#d49a2e`)

**Status & Data Sequences (Darkened)**
* `--status-1`: `#218358` (Green)
* `--status-2`: `#2271a1` (Blue)
* `--status-3`: `#a86520` (Amber)
* `--status-4`: `#8b4fc0` (Purple)
* `--status-5`: `#cd2b31` (Red)
* `--status-premium`: `#9a7800` (Gold)

**Borders**
* `--border` (`#d8d9e0`)
* `--border-hover` (`#bcbdc8`)

**Elevation (Softer for light context)**
...
**Key rule:** Never reference raw color hex values in components. Always use `var(--token-name)` so themes propagate automatically.
